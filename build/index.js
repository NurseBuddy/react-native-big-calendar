...
function todayInMinutes() {
  return getDateDiffMinutes(new Date(), dayjs__default['default']().startOf('day').toDate())
}
...
function getDateDiffMinutes(a, b) {
  var ret = (a.valueOf() - b.valueOf()) / 1000 / 60
  if (ret >= 0) return ret
  return -ret
}
function getDateDiffDays(a, b) {
  var ret = (a.valueOf() - b.valueOf()) / 1000 / 60 / 60 / 24
  if (ret >= 0) return ret
  return -ret
}
function getEventSpanningInfo(event, date, dayOfTheWeek, calendarWidth, showAdjacentMonths) {
  ...
  var eventDuration = Math.floor(getDateDiffDays(event.start, event.end)) + 1
  var eventDaysLeft = Math.floor(getDateDiffDays(event.end, date.toDate())) + 1
...
function getEventMap(events) {
  ...
  events.forEach(function (event) {
    ...
    var startDayjs = dayjs__default['default'](start)
    var endDayjs = dayjs__default['default'](end)
    while (startDayjs <= endDayjs) {
      var eventToAdd = void 0
      var morning = startDayjs.startOf('day').format()
      var tomorrowMorning = startDayjs.add(1, 'day').startOf('day')
      // if event spans over to next day, split event at midnight
      if (tomorrowMorning > endDayjs) {
        eventToAdd = __assign(__assign({}, event), {
        ...
        eventToAdd = __assign(__assign({}, event), {
          start: startDayjs.toDate(),
          end: tomorrowMorning.toDate(),
        })
      }
      if (!eventMap.has(morning)) {
        eventMap.set(morning, [eventToAdd])
      } else {
        var array = eventMap.get(morning) || []
...
var getEventCellPositionStyle = function (start, end) {
  var relativeHeight = 100 * (1 / DAY_MINUTES) * getDateDiffMinutes(start, end)
...
function _CalendarBody(_a) {
  ...
  var eventMap = React__namespace.useMemo(
    function () {
      return getEventMap(events)
    },
    [events],
  )
      ...
      React__namespace.createElement(
        ...
        dateRange.map(function (date) {
          return React__namespace.createElement(
            ...
            (eventMap.get(date.startOf('day').format()) || []).map(_renderMappedEvent),
            isToday(date) &&
              !hideNowIndicator &&
              React__namespace.createElement(reactNative.View, {
                style: [
...
function _CalendarBodyForMonthView(_a) {
  ...
  var eventMap = getEventMap(events)
  var sortedEvents = React__namespace.useCallback(
    function (day) {
      var events = eventMap.get(dayjs__default['default'](day).startOf('day').format()) || []
      if (sortedMonthView) {
        var ret = events.sort(function (a, b) {
...
function _CalendarContainer(_a) {
  var events = _a.events,
    ...
    _0 = _a.moreLabel,
    moreLabel = _0 === void 0 ? '{moreCount} More' : _0,
    _1 = _a.showAdjacentMonths,
    showAdjacentMonths = _1 === void 0 ? true : _1,
    _2 = _a.sortedMonthView,
    sortedMonthView = _2 === void 0 ? true : _2,
    _3 = _a.hideHours,
    hideHours = _3 === void 0 ? false : _3
  var _4 = React__default['default'].useState(dayjs__default['default'](date)),
    targetDate = _4[0],
    setTargetDate = _4[1]
  React__default['default'].useEffect(
    function () {
  ...
  var getDateRange = React__default['default'].useCallback(
    function (date) {
      switch (mode) {
        case 'month':
...
exports.getDateDiffDays = getDateDiffDays
exports.getDateDiffMinutes = getDateDiffMinutes
